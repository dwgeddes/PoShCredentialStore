function Get-StoredCredentialPlainText {
    ...
    process {
        ...
        try {
            ...
            # Validate the credential object structure
            if (-not $storedCred.Credential) {
                Write-Error "Stored credential '$Name' has no Credential property"
                return @()
            }
            Write-Verbose "Credential property exists, type: $($storedCred.Credential.GetType().Name)"
            
            # Check if password SecureString is null 
            if ($null -eq $storedCred.Credential.Password) {
                Write-Error "Stored credential '$Name' has null password data"
                return @()
            }
            Write-Verbose "Password SecureString exists, type: $($storedCred.Credential.Password.GetType().Name)"
            
            # Convert SecureString to plain text using working helper function
            Write-Verbose "Converting SecureString to plain text"
            $plainPassword = ConvertFrom-SecureStringToPlainText -SecureString $storedCred.Credential.Password
